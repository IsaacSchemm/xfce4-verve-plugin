...
typedef struct
{
  ...
  static int num_engines = 7;
  /* The unique IDs for these search engines were generated by piping "echo -n [name]" into the GNU cksum tool.
     You can add to these list, but it should be updated in verve.c as well. Also, don't change a well-established
     ID, because this is stored in the user's rc file. */
  static char* engine_names[7] = {"No; launch local commands", "DuckDuckGo (!ddg)", "Google (!g)", "Bing (!b)", "Wikipedia (!w)", "Wolfram|Alpha (!wa)", "Debian man pages (!dman)"};
  //                       none, DuckDuckGo       Google        Bing    Wikipedia  Wolfram|Alpha  Debian man pages
  static int engine_ids[7] = {0, 1667375360, -1822996848, 2002029412, -2140183184,    1140400801,      -1091169193};
                           ...
                           VervePlugin *verve)
{
  ...
  /* Update search engine ID */
  verve_plugin_update_search_engine (NULL, engine_ids[gtk_combo_box_get_active(box)], verve);
}
                         ...
                         VervePlugin *verve)
{
  ...
  for (i=0; i<num_engines; i++) {
    gtk_combo_box_append_text(GTK_COMBO_BOX(engine_box), engine_names[i]);
  }
  ...
  i = 0;
  while (!found && (i<num_engines)) {
    if (engine_ids[i] == current_id) {
      found = TRUE;
      gtk_combo_box_set_active (GTK_COMBO_BOX (engine_box), i);
    } else {
      i++;
    }
  }
